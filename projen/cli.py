...
@click.group()
@click.option(
    "--no-telemetry", is_flag=True, default=False,
...
@click.option(
    '--build', type=click.Choice(['bazel', 'gradle', 'both']),
    default=DEFAULTS['build'], help='Build system to scaffold'
)
@click.option(
    '--language', type=click.Choice(DEFAULTS['languages']),
    required=True, help='Primary project language'
)
@click.option(
    '--license', default=DEFAULTS['license'],
    help='License identifier (e.g., MIT, Apache-2.0)'
)
@click.option(
    '--ci', type=click.Choice(DEFAULTS['ci_providers']),
    default=DEFAULTS['ci'], help='CI provider to configure'
)
@click.option(
...
def init(ctx, project_name, build, language, license, ci, overwrite):
    ...
    click.echo(f"Scaffolding '{project_name}' with build={build}, language={language}, license={license}, ci={ci}\nOverwrite: {overwrite}")
    try:
        scaffold_project(
